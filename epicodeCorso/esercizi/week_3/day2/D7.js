...
function concatenateStrings(string1, string2) {
  string1 = string1.slice(1, 3);
  string2 = string2.slice(3, 6)
...
select.addEventListener('change', (event) => {
  moviedetails.innerHTML = `Il film scelto ha il codice ${event.target.name}`;
});

window.addEventListener('load', filmSelect(movies));
/*
select.addEventListener('change', function (movies) {
  movies.forEach(element => {
    details = ` Il tuo film è ${element.Title}, uscito l'anno ${element.Year}.`;
    return details;
  })
