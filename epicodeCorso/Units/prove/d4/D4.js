...
var totalMass = 0;
{
  ...
  while (x < starWarsCharacters.length) {
    totalMass += parseInt(starWarsCharacters[x].mass);
    x++
  }
...
for (z = 0; z < starWarsCharacters.length; z++) {
  if (starWarsCharacters[z].gender === 'n/a') {
    starWarsCharacters[z].gender = 'robot';
  }
}
...
for (y = 0; y < characters.length; y++) {
  for (i = 0; i < femaleCharacters.length; i++) {
    if (characters[y] == femaleCharacters[i].name) {
...
var casualCharacter = RandArray(starWarsCharacters);
console.log(casualCharacter);